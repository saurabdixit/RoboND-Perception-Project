...
```
9. Wait for the robot to move its hand away from the scene. Then run following command, enable point cloud, and markers in rviz window to see the object detection.
```bash
rosrun pr2_robot pr2_perception.py
```

10. output_data_world\_\#.yaml will be saved to following location
```bash
roscd pr2_robot/config
```
11. Thanks for running the project
...
compute_color_histograms() and compute_normal_histograms() is written in the features.py code. For more details, please open features.py
```bash
rosed pr2_robot features.py
```
